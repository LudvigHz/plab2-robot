...
class Imager:
    ...
    def load_image(self):
        self.image = Image.open(
            self.fid
        )  # the image is actually loaded as needed (automatically by PIL)
    ...
    def map_image(self, func, image=False):
        ...
        image = image if image else self.image
        return Imager(
            image=Image.eval(image, func)
        )  # Eval creates a new image, so no need for me to do a copy.
    ...
    def morph(self, im2, alpha=0.5):
        ...
        for x in range(self.xmax):
            for y in range(self.ymax):
                rgb = self.combine_pixels(
                    self.get_pixel(x, y), im2.get_pixel(x, y), alpha=alpha
                )
...
def ptest1(
    fid1="images/kdfinger.jpeg", fid2="images/einstein.jpeg", steps=5, newsize=250
):
    im1 = Imager(fid1)
    im2 = Imager(fid2)
...
def ptest2(
    fid1="images/einstein.jpeg",
    outfid="images/tunnel.jpeg",
    levels=3,
    newsize=250,
    scale=0.8,
):
